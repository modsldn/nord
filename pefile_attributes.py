""" PE Features: This class pulls static features
    ...
    Reference: https://github.com/SuperCowPowers/data_hacking/blob/master/pefile_classification/pe_features.py
    # Author: Brian Wylie
"""
import pefile
...
class PEFileFeatures:
    ...
    def __init__(self):
        ...
        self.set_sparse_features(
            ["imported_symbols", "section_names", "pe_warning_strings"]
        )
    ...
    def open_using_pefile(self, input_name, input_bytes):
        ...
        try:
            ...
            print(
                f"warning: pe_fail (with exception from pefile module) on file: {input_name}"
            )
            error_str = f"(Exception):, {str(error)}"
            return None, error_str

        # Now test to see if the features are there/extractable if not return FAIL flag
        if (
            pe.PE_TYPE is None
            or pe.OPTIONAL_HEADER is None
            or len(pe.OPTIONAL_HEADER.DATA_DIRECTORY) < 7
        ):
            print(f"warning: pe_fail on file: {input_name}")
            error_str = f"warning: pe_fail on file: {input_name}"
            return None, error_str
    ...
    def extract_features_using_pefile(self, pe):
        ...
        if hasattr(pe, "DIRECTORY_ENTRY_BOUND_IMPORT"):
            extracted_dense["number_of_bound_imports"] = len(
                pe.DIRECTORY_ENTRY_BOUND_IMPORT
            )
            num_imported_symbols = 0
        ...
        if hasattr(pe, "DIRECTORY_ENTRY_EXPORT"):
            extracted_dense["number_of_export_symbols"] = len(
            ...
            for symbol in pe.DIRECTORY_ENTRY_EXPORT.symbols:
                ...
                if not symbol.name:
                    ...
                    symbol_info = "name=" + str(symbol.name)
        ...
        if hasattr(pe, "DIRECTORY_ENTRY_IMPORT"):
            ...
            for module in pe.DIRECTORY_ENTRY_IMPORT:
                for symbol in module.imports:
                    ...
                    symbol_set.add(
                        convertToUTF8("%s:%s" % (module.dll, symbol_info)).lower()
                    )
        ...
        for feature in self._dense_feature_list:
            if extracted_dense[feature] == feature_not_found_flag:
                ...
                if self._verbose:
                    self.log(
                        "info: Feature: %s not found! Setting to %d"
                        % (feature, feature_default_value)
                    )
                    self._warnings.append(
                        "Feature: %s not found! Setting to %d"
                        % (feature, feature_default_value)
                    )
...
def test():
    ...
    with open("data/1/0aBz8OelJjk4amYzVBBUVJ3cR7tiHAoX.exe", "rb") as f:
